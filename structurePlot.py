...
#    colors1 = ['#3686B7', '#C9BA83', '#7f1f17', '#3CB371'] #red, yellow, blue, green
    colors1 = ['#FE4365', '#F9CDAD', '#83AF9B', '#C8C8A9']
#    colors2 = px.colors.qualitative.Vivid
    colors2 = ['#FE4365', '#FC9D9A', '#EDE574', '#F9CDAD', '#C06C84', '#C8C8A9', '#45ADA8', '#9DE0AD'][::-1]
    colorOrder = {(2,0):0,(2,1):2,(3,0):1,(3,1):2,(3,2):0,(4,0):0,(4,1):1,(4,2):3,(4,3):2}
    fig = make_subplots(rows=4, cols=1, row_heights=[0.07, 0.305, 0.305, 0.305], vertical_spacing=0.005)
    for k in range(2, 4+1):
        d, vbounds, vbounds2 = readData(k)
#    d.to_csv('/dev/stdout', sep='\t', index=None)
        for i in range(d.shape[1]-5):
            color = colors1[colorOrder[(k,i)]]
            fig.add_trace(go.Bar(x=d['inv'], y=d.iloc[:,3+i], showlegend=False, marker_color=color), row=k, col=1)

    fig.add_trace(go.Scatter(x=[10], y=[2], mode='text', text=['A'], textfont=dict(family=fontStyle, size=400, color='white'), showlegend=False), row=1, col=1)
    fig.add_trace(go.Scatter(x=[(j[0]+j[1])/2 for j in vbounds], y=[1.5]*len(vbounds), mode='text', text=[ '<b>'+i+'</b>' for i in clusters ], textfont=dict(family=fontStyle, size=440, color='black'), showlegend=False), row=1, col=1)
    ...
    vbounds2['x'] = (vbounds2['s']+vbounds2['e'])/2
    fig.add_trace(go.Scatter(x=vbounds2['x'], y=[0.3]*vbounds2.shape[0], text=vbounds2['pop'], mode='text', textfont=dict(family=fontStyle, size=400)))
    ...
    fig.update_layout(barmode='stack', template='plotly_white', height=16000, width=16000)
#    fig.update_xaxes(titlefont=None, tickfont=dict(family=fontStyle, size=fontSize['tick']-9), tickangle=90, row=1, col=1)
    ...
    fig.update_xaxes(titlefont=None, showticklabels=True, tickfont=dict(family=fontStyle, size=fontSize['tick']), showgrid=False, showline=False, row=4, col=1)
...
#    fig.update_yaxes(title='K='+str(k), titlefont=dict(family=fontStyle, size=fontSize['title']), showgrid=False, tickwidth=0, showticklabels=False)
    pio.write_image(fig, 'frappe.jpg')
